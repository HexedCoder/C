...
static const int point[TEST_TOTAL] = {2, 1, 1,  1, 1, 1, 2,
                                      3, 3, 17, 2, 1, 1, 4};
...
int *random_array(int n) {
  ...
  //
  // Initialize array from 0 to n - 1, then randomly permute to avoid
  // duplicates.
...
int main(void) {
  ...
  // Test.
  puts(section);
  bst_insert(bst, create_node(25));
  bst_insert(bst, create_node(29));
#ifdef DEBUG
  print(bst);
#endif
  ...
  // Test.
  puts(section);
  bst_insert(bst, create_node(22));
  bst_insert(bst, create_node(38));
#ifdef DEBUG
  print(bst);
#endif
  ...
  for (int i = 1; i < BIGN; ++i) {
    bst_insert(bst, create_node(rarray[i]));
  }
  gettimeofday(&t2, 0);
  time = (t2.tv_sec - t1.tv_sec) + (t2.tv_usec - t1.tv_usec) / 1000000.0;
  ...
  time = test_delete_runtime(bst, &tree_delete, 0.05);
  printf("Time: %f seconds\n", time);
  puts(footer);
  // End of exercise.
  report();
